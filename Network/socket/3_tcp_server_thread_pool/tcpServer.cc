...
void transService(int sock, const std::string &clientIP, uint16_t clientPort) {
  ...
  while (true) {
    // 读取字节流直接用read
    ssize_t s =
        read(sock, inbuffer, sizeof(inbuffer) - 1);  // 我们认为读到的都是字符串
    if (s > 0) {
      // read success
      inbuffer[s] = '\0';
      if (strcasecmp(inbuffer, "quit") ==
          0)  // strcasecmp是无视大小写进行字符串比较
      {
        logMessage(DEBUG, "client quit -- %s[%d]", clientIP.c_str(),
                   clientPort);
        break;
      }
      logMessage(DEBUG, "trans before: %s[%d]>>> %s", clientIP.c_str(),
                 clientPort, inbuffer);
      // 提供服务
      for (int i = 0; i < s; i++) {
      ...
      logMessage(DEBUG, "trans after: %s[%d]>>> %s", clientIP.c_str(),
                 clientPort, inbuffer);
      write(sock, inbuffer, strlen(inbuffer));
    }
    // 差错控制
    else if (s == 0) {
      // pipe: 读端一直在读，写端不写了，并且关闭了写端，会读到
      // s==0，代表对面端口关闭了 s==0 代表对方关闭，client退出
      logMessage(DEBUG, "client quit -- %s[%d]", clientIP.c_str(), clientPort);
      break;
    } else {
      // 读取出错
      logMessage(DEBUG, "%s[%d] - read: %s", clientIP.c_str(), clientPort,
                 strerror(errno));
      break;
    }
  }
...
void execCommand(int sock, const std::string &clientIP, uint16_t clientPort) {
  ...
  while (true) {
    // 读取字节流直接用read
    ssize_t s =
        read(sock, command, sizeof(command) - 1);  // 我们认为读到的都是字符串
    if (s > 0) {
      command[s] = '\0';
      ...
      */
      ...
      if (fp == nullptr) {
        logMessage(WARNING, "exec %s failed, beacuse: %s", command,
                   strerror(errno));
        break;
      }
      ...
      // fflush(fp);
      pclose(fp);
      logMessage(DEBUG, "[%s:%d] exec [%s] ... done", clientIP.c_str(),
                 clientPort, command);
    }
    // 差错控制
    else if (s == 0) {
...
class ServerTcp {
 ...
 public:
  void init() {
    ...
    if (bind(_listenSock, (const struct sockaddr *)&local, sizeof(local)) ==
        -1) {
    ...
    // 3.
    // 监听socket，tcp是面向连接的。所谓面向的意思就是在做任何事情之前先干什么
    // 比如面向对象就是先要创建对象
  ...
  void loop() {
    _tp->start();
    logMessage(DEBUG, "thread pool start success, thread num: %d",
               _tp->threadNum());
    ...
    while (!_quit) {
      ...
      // SIG_IGN
      // 忽略，子进程在终止时会自动清理掉,不会产生僵尸进程，也不会通知父进程
      ...
      logMessage(DEBUG, "accept: %s | %s:%d, socket fd: %d", strerror(errno),
                 peerIP.c_str(), peerPort, serviceSock);
