...
inline void comm_call_handlers(int fd, int read_event, int write_event) {
  ...
  debug(5, 8)(
      "comm_call_handlers(): got fd=%d read_event=%x write_event=%x "
...
static void checkTimeouts(void) {
  ...
  for (fd = 0; fd <= Biggest_FD; fd++) {
    ...
    */
    ...
    if (F->timeout == 0) continue;
    if (F->timeout > sys_curtime) continue;
...
int comm_select(int msec) {
  ...
  if (msec > MAX_POLL_TIME) msec = MAX_POLL_TIME;
  ...
  if (last_timeout + 0.999 < current_dtime) {
    ...
    if (max_timeout < msec) msec = max_timeout;
  }
  ...
  if (rc == COMM_TIMEOUT) debug(5, 8)("comm_select: time out\n");
