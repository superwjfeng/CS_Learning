...
class TcpServer {
 ...
 public:
  ...
  void AddConnection(int sock, func_t recv_cb, func_t send_cb,
                     func_t except_cb) {
    ...
    conn->_tsvr = this;
    // 2. 添加sock到epoll中
    _poll.AddSockToEpoll(
        sock,
        EPOLLIN |
            EPOLLET);  // 任何多路转接的服务器，一般默认只会打开对读取事件的关心，写入事件会按需进行打开！
  ...
  void Accepter(Connection *conn) {
    ...
    while (true) {
      ...
      // sock一定是常规的IO sock
      int sock =
          Sock::Accept(conn->_sock, &clientip, &clientport, &accept_errno);
      if (sock < 0) {
        if (accept_errno == EAGAIN || accept_errno == EWOULDBLOCK)
          ...
          // accept失败
          logMessage(WARNING, "accept error, %d : %s", accept_errno,
      ...
      if (sock >= 0) {
        AddConnection(
            sock, std::bind(&TcpServer::Recver, this, std::placeholders::_1),
            std::bind(&TcpServer::Sender, this, std::placeholders::_1),
            std::bind(&TcpServer::Excepter, this, std::placeholders::_1));
        logMessage(
            DEBUG,
            "accept client %s:%d success, add to epoll&&TcpServer success",
